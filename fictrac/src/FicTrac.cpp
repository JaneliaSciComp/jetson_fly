...
void* socketListener(void* params)
{
    ...
    // WBD set socket to fixed port number
    // --------------------------------------------------------
    // serv_addr.sin_port = htons(SOCKET_SIN_PORT);
    // --------------------------------------------------------
    ...
    while( ACTIVE ) {
        ...
        // WBD fix this? .... socket always closed? why not stay open
        // -----------------------------------------------------------
        close(newsockfd);
        newsockfd = -1;
        // -----------------------------------------------------------
...
int main(int argc, char *argv[])
{
    ...
    ///
    {
        printf("\n####################################################################\n");
        printf("#                                                                  #\n");
        printf("# FicTrac: A webcam-based method for generating fictive paths.     #\n");
        printf("# Version: public release (build %s)                      #\n", __DATE__);
        printf("# Copyright (C) 2011-2017 Richard Moore (rjdmoore@uqconnect.edu.au)#\n");
        printf("#                                                                  #\n");
        printf("####################################################################\n");
        printf("#                                                                  #\n");
...
//  startWindowThread();
    ...
    double sphere_cx = -1, sphere_cy = -1, sphere_rad = -1;
    ...
    ifstream file(config_fn.c_str(),ifstream::in);
    if( !file.is_open() ) {
        fprintf(stderr, "%s: Error, unable to open file (%s)!\n", __func__, config_fn.c_str());
        fflush(stdout);
        exit(-1);
    }

    string config_dir;
    size_t slash = config_fn.find_last_of("/\\");
    if( slash != string::npos ) {
        config_dir = config_fn.substr(0,slash) + '/';
        printf("Setting config directory to: %s\n", config_dir.c_str());
    }
    ...
    while(!file.eof()) {
        ...
        if( tokens.size() > 1 ) {
            if( tokens.front().compare("input_vid_fn") == 0 ) {
                tokens.pop_front();
                input_vid_fn = config_dir + tokens.front();
            } else if( tokens.front().compare("output_fn") == 0 ) {
                tokens.pop_front();
                output_fn = config_dir + tokens.front();
            } else if( tokens.front().compare("mask_fn") == 0 ) {
                tokens.pop_front();
                mask_fn = config_dir + tokens.front();
            } else if( tokens.front().compare("transform_fn") == 0 ) {
                tokens.pop_front();
                transform_fn = config_dir + tokens.front();
            } else if( tokens.front().compare("template_fn") == 0 ) {
                tokens.pop_front();
                template_fn = config_dir + tokens.front();
            } else if( tokens.front().compare("closed_loop_fn") == 0 ) {
                tokens.pop_front();
                closed_loop_fn = config_dir + tokens.front();
            ...
            } else if( tokens.front().compare("sphere_centre_px") == 0 ) {
                        tokens.pop_front();
                        sphere_cx = Utils::STR2NUM(tokens.front());
                        tokens.pop_front();
                        sphere_cy = Utils::STR2NUM(tokens.front());
            } else if( tokens.front().compare("sphere_radius_px") == 0 ) {
                        tokens.pop_front();
                        sphere_rad = Utils::STR2NUM(tokens.front());
            }
        }
    ...
    if( (thresh_win <= 0) || (thresh_win >= 1) ) {
        fprintf(stderr, "ERROR: thresh_win is now defined as a %% of the ROI window and must be within the range (0,1)!\n");
        fflush(stderr);
        exit(-1);
    }
    ...
    printf("sphere_centre_px:.  %f %f\n", sphere_cx, sphere_cy);
    printf("sphere_radius_px:.  %f\n", sphere_rad);
    printf("\n");
    ...
    if( do_config ) {
        ...
        if (sphere_cx >= 0 && sphere_cy >= 0) {
            ...
            if (sphere_rad > 0) {
                ...
                cam_model->pixelIndexToVector(sphere_cx + sphere_rad, sphere_cy, sphere_circum);
...
//  }
    ...
    MatchSphereAbs sphere(
            ...
            (do_display || save_video), do_update,
            -nlopt_res, nlopt_res,
...
std::cout << "Finished creating channels for Voltage output\n";
    ...
    for( unsigned int cnt = 1, seq_n = 1; ; cnt++, seq_n++ ) {
        ...
        memcpy(guess, av_guess, 3*sizeof(double));
...
//          guess[2] = Utils::GEN_RAND_DBL(-0.01, 0.01);
            ...
            printf("initial guess:\t\t%.3f %.3f %.3f\n",
                    guess[0], guess[1], guess[2]);


            // WBD
            //sphere.optimize(guess);
            nlopt_result optimize_result = sphere.optimize(guess);
            sphere.getOptX(guess);
            err = sphere.getOptF();

            bad_frame = err > max_err;

            printf("initial minimisation:\t%.3f %.3f %.3f  (err=%.3f, opt_res=%d)\n",
                    guess[0], guess[1], guess[2], err, optimize_result);
...
//                  if( !bad_frame ) { break; }
        ...
        if( !bad_frame || !SPHERE_INIT ) {
            ...
            printf("optimum rotation:\t%.3f %.3f %.3f  (err=%.3f/its=%d)\n",
...
//          out[7] = 0x4D;
        ...
        data_msg["velx"] = velx;
        data_msg["vely"] = vely;
        data_msg["heading"] = heading*Maths::R2D;
...
//                      int b = int(255.0*(1.0-i/float(hist_length))+0.5);
            ...
            char times[128];
            time_t t = time(NULL);
            strftime(times, 22, "Processed %d/%m/%Y", localtime(&t));
            ...
            drawText(draw.data, draw.cols, draw.rows,
                    draw.channels(), draw.step,
                    string("FicTrac (") + string(__DATE__) + string(")"),
                    draw.cols-184, 15, 1.0, 0, 255, 255, true, -1);
            drawText(draw.data, draw.cols, draw.rows,
                    draw.channels(), draw.step,
                    string("input image"),
